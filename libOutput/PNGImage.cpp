...
bool PNGImage::writeFileData(const std::string& filename, const int width,
		const int height, const float *data) {
	int rc, error = 0;
	ulg rowbytes;
	...
	if (m_png_fileinfo.interlaced) {
		...
		m_png_fileinfo.row_pointers = (uch **) malloc(m_png_fileinfo.height
				* sizeof(uch *));
		if (m_png_fileinfo.image_data == NULL || m_png_fileinfo.row_pointers
				== NULL) {
		...
		for (i = 0; i < m_png_fileinfo.height; ++i)
			m_png_fileinfo.row_pointers[i] = m_png_fileinfo.image_data + i
					* rowbytes;
		bytes = fread(m_png_fileinfo.image_data, 1, image_bytes,
				m_png_fileinfo.infile);
		...
		for (j = m_png_fileinfo.height - 1; j >= 0L; --j) {
			...
			for (x = 0; x < rowbytes; x += 3) {
				m_png_fileinfo.image_data[x] = (uch) (floor(data[j * width * 3
						+ pixelcount + 0] * 255));
				m_png_fileinfo.image_data[x + 1] = (uch) (floor(data[j * width
						* 3 + pixelcount + 1] * 255));
				m_png_fileinfo.image_data[x + 2] = (uch) (floor(data[j * width
						* 3 + pixelcount + 2] * 255));
				pixelcount += 3;
